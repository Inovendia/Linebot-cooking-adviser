...
def handle_event(event):
    ...
    if isinstance(event, PostbackEvent):
        ...
        if postback_data_t != "送信中" and postback_data_t != "これから提案するから、少し待っててね":
                #postback_data = user_postback_data.get(user_id, {})
...
#---------------------------------------------------------------------------------------------
    ...
    #                 text='▼ タップして選ぶ ▼',
    #                 actions=[
    #                     PostbackAction(label="いつも通り", data="いつも通り", text="送信中"),
    #                     PostbackAction(label="楽したい", data="楽したい", text="送信中"),
    #                     PostbackAction(label="元気になりたい", data="元気になりたい", text="送信中"),
    #                     PostbackAction(label="凝りたい", data="凝りたい", text="送信中")
    #                 ]
    ...
    # elif postback_data in ["いつも通り","楽したい","元気になりたい","凝りたい"]:
    #     if user_id not in user_data:
    #         user_data[user_id] = {}
    ...
    #     )
    #     print(user_data)####
    # #保存されたpostback_dataを取得
    # # postback_data = user_postback_data.get(user_id, None)
    # # if postback_data:
    # # #chatGPTに送るメッセージ
    # elif postback_data_t != "送信中" and postback_data_t != "少し待っててね" :
    #         #postback_data = user_postback_data.get(user_id, {})
    #         #feeling = postback_data.get("feeling","普通")
    #         try:
    #             messagess = [
    #             {"role": "system", "content": f"あなたはベテランの主婦で料理のレパートリーも多いです。今の私の気分は{feeling}です。料理を1つ提案して下さい。"},
    #             {"role": "user", "content": postback_data_t},
    #             ]
    #             response = openai.chat.completions.create(
    #                 model="gpt-3.5-turbo",
    ...
    #     shell = "貝をよく"
    #     quick_reply_buttons = [
    #     QuickReplyButton(action=PostbackAction(label=balance,data=balance,text="これから提案するから、少し待っててね")),
    #     QuickReplyButton(action=PostbackAction(label=bird,data=bird,text="これから提案するから、少し待っててね")),
    #     QuickReplyButton(action=PostbackAction(label=pork,data=pork,text="これから提案するから、少し待っててね")),
    #     QuickReplyButton(action=PostbackAction(label=beef,data=beef,text="これから提案するから、少し待っててね")),
    #     QuickReplyButton(action=PostbackAction(label=shell,data=shell,text="これから提案するから、少し待っててね")),
    #     QuickReplyButton(action=PostbackAction(label=fish,data=fish,text="これから提案するから、少し待っててね"))
    #     ]
    #     quick_reply = QuickReply(items = quick_reply_buttons)
    #     quick_message = TextSendMessage(
    #         text = "肉類・魚介類で好みがあれば教えて下さい",
...
#     user_input=event.message.text
#     feeling = user_postback_data.get(user_id, {}).get("feeling", None)
...
#     elif user_input == "これから提案するから、少し待っててね":
#         user_info = user_data.get(user_id,{})
#         budget = user_info.get("budget","未設定")
#         flavor = user_info.get("flavor","未設定")
#         important = user_info.get("important","未設定")
...
#                 "content":
#                 f"あなたはベテランの主婦で料理のレパートリーも多いです。"
#                 f"私は{user_info['flavor']}が好きで、{user_info['spice']}が効いたものが好きです。"
#                 f"料理をする時は{user_info['important']}に気を遣っています。"
#                 f"私が好きな料理のジャンルは{user_info['Genre']}です。"
#                 f"肉と魚は{user_info['meat&fish']}使います。"
#                 f"私の好みに合うように、予算{user_info['budget']}で1週間の料理の献立を提案して下さい。"
#             }
#         ]
#         try:
#             response = openai.chat.completions.create(
...
#     else:
#         line_bot_api.reply_message(event.reply_token, TextSendMessage(text="気分を選んでからメッセージを入力して下さい。"))



#----------------------------------------------------↓↓元々コメントアウトされていた--------------------------
# @handler.add(MessageEvent, message = QuickReply)
# def handle_quick_reply(event):
#     user_message = event.message.text
#     user_id = event.source.user_id
#     #reply_token = event.reply_token

#     if user_message in ["甘いもの","辛いもの","酸っぱいもの","しょっぱいもの","苦いもの"]:
#         user_postback_data[user_id] = user_message
#         print(user_message)
#         messages = [
#             {"role": "system", "content": f"あなたはベテランの主婦で料理のレパートリーも多いです。私の好きな料理の味は{user_message}です。料理を1つ提案して下さい。"}
